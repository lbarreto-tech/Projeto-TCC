...
public interface AgendamentoRepository extends JpaRepository<Agendamento, Long> {
     ...
     */
    @Query("SELECT COUNT(a) > 0 FROM Booking a " +
     ...
     */
    @Query("SELECT a.dataEvento FROM Booking a WHERE a.status <> 'REJEITADO'")
    List<LocalDate> buscarTodasDatasOcupadas();

    /**
     * Busca agendamentos cuja data do evento já passou, para limpeza automática.
     */
    @Query("SELECT a FROM Booking a WHERE a.dataEvento < :hoje")
